...
那么对于`testdb`中未显示指定字符集的表或者字段，将默认使用`latin1`字符集, `test`表使用`utf8`字符集，表中各字段的字符集如下：
| 字段名 | 字符集 |
| ---- | ---- |
| latin_name | latin1 |
...
| default_name | utf8 |
## utf8与utf8mb4
细心的朋友一定注意到了，这里出现了个一个叫`utf8mb4`的字符集，这个才是真正的`utf8`字符集，而MySQL中的`utf8`实际上是一个阉割版的`utf8`字符集，它是`utf8mb3`，他每个字符最长只有3字节，是不完全的，这是MySQL的一个历史问题。
## 查询中字符集的转换过程
上面提到，MySQL中存在很多字符集变量，这里重点介绍下`character_set_connection`、`character_set_client`以及`character_set_results`，这三个变量涉及到Query请求的字符集转换过程，整个过程如下图
...
+-------------+--------------+
```
我们可以看出虽然`latin1_name`和`default_name`分是`latin1`和`utf8`编码，但，我们对于中文`红`的存储都能够正确读取和显示。
...
首先Client将其编码成`utf8`传输到MySQL Server，而MySQL Server却认为这是`latin1`，由于存储数据使用`utf8`字符集，这时MySQL Server把其当做`Latin1`字节流，再次对齐进行`utf8`编码。

上面说的二次编码问题，不会对显示造成什么影响，但是如果需要对该字段进行排序，可能会导致，排序结果不符合你的预期。
## 比较规则
...
我们可能会认为比较规则没什么意义，比如中文字符`我`的UTF8编码为`&#x6211;`既然有了比较两段字符是否相等呢
`
# 总结
